...
import { ROUTES, TransactionType, type Category } from "~/utils/constants";
...
export default function CategoriesPage() {
  ...
  const isDeleting =
    ...
    fetcher.formData?.get("categoryId") === categoryToDelete?.id &&
    fetcher.formAction === "/api/expense/categories/delete";
  ...
  const closeCreateModal = (newCat:any) => {
  ...
  const CategoryList = ({
    ...
    <div className="bg-white relative dark:bg-gray-800 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50 h-full">
      <h2 className={`text-xl font-bold mb-4 ${colorClass}`}>{title}</h2>
      <div className="space-y-3 mb-16">
        {items.map((cat) => (
          <div
            key={cat.id}
            className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl group transition-all hover:shadow-sm"
          >
            {editingId === cat.id ? (
              <>
                <input
              ...
              <>
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-2 text-gray-400 hover:text-indigo-600 hover:bg-white dark:hover:bg-gray-600 rounded-lg transition-colors"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
                  ...
                  <button
                    ...
                    className="p-2 text-gray-400 hover:text-red-600 hover:bg-white dark:hover:bg-gray-600 rounded-lg transition-colors"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
      ...
      <div className="w-full flex justify-center py-5 absolute bottom-0">
        <button
          onClick={() => {
            setType(title.split(" ")[0].toLowerCase());
            setIsAddModalOpen(true);
          }}
          className={`px-4 py-2 bg-gray-600 text-white rounded-4xl shadow-lg shadow-${colorClass}-500/30 hover:bg-indigo-700 hover:scale-105 transition-all font-medium text-sm`}
        >
          +
        </button>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {isAddModalOpen && (
          <div className="fixed inset-0 z-10 flex items-center justify-center p-4 sm:p-6">
            ...
            ></div>
            <CreateCategory type={type} onCreate={closeCreateModal} />
          </div>
        )}
          ...
          message={
            <span>
              ...
              <span className="text-xs">
                The category won't be selectable anymore.
              </span>
            </span>
          }
