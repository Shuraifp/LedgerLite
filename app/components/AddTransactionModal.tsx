...
import { TransactionType, type Category } from "~/utils/constants";
...
export function AddTransactionModal({
  ...
  const [view, setView] = useState<
    "MAIN" | "SELECT_CATEGORY" | "CREATE_CATEGORY"
  >("MAIN");
    ...
    if (isOpen) {
      if (initialData) {
        ...
        setSelectedCategoryId(
          categories.find((c) => c.name === initialData.categoryName)?.id || ""
        );
      } else {
        setAmount("");
        setTitle("");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-3xl shadow-2xl overflow-hidden transform transition-all scale-100 max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 sm:p-8 pt-2 overflow-y-auto custom-scrollbar">
          {view === "CREATE_CATEGORY" && <CreateCategory type={type} onCreate={handleCreateCategory} />}
                ...
                <div className="space-y-1 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                  ...
                  {filteredCategories.map((cat) => (
                    ...
                    >
                      ...
                      {selectedCategoryId === cat.id && (
                        <svg
                          className="w-5 h-5 ml-auto text-indigo-500"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
          ...
          {view === "MAIN" && (
            <fetcher.Form
              ...
              action={
                isEditing
                  ? "/api/expense/transactions/update"
                  : "/api/expense/transactions/create"
              }
